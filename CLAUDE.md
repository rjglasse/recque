...
Single-user, single-process app with two equal front ends: the Textual TUI (`recque_tui/ui`) and the FastAPI web app (`recque_web/`). Both go through the same core and application services. Original prototypes live in `legacy/` (frozen).

**Read `VISION.md` before planning or picking work.** It says who recque is for (students), what "better" means (the evaluation harness), where agents may take it, and what is reserved for Ric.

## How agents work here

recque is a project where **agents plan the direction** (see `VISION.md`, "How agents work here"). In short:

- Agents own the backlog: create, split, prioritise, defer and close issues against `VISION.md`, with reasoning in issue notes.
- Agents **push to `main`** once `uv run pytest -q` and `uv run ruff check .` pass: `git pull --rebase` first, never force-push or rewrite history, and `bd dolt push` alongside `git push`. This deliberately overrides Ric's usual rule that nothing merges without him.
- Only Ric edits `VISION.md`. Disagreement goes in an issue labelled `vision`; ideas outside the listed directions go in an issue labelled `idea`.
- Ric's decisions (anything `VISION.md` reserves for him, live-model spend over $1 per evaluation run, licence, releases, anything involving real students) go to him through `bd human`.
